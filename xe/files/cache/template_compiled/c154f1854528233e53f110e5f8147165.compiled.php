...
<?php if($__Context->layout_info->colorset=='default' || !$__Context->layout_info->colorset){ ?><!--#Meta:layouts/xe_official/css/default.css--><?php $__tmp=array('layouts/xe_official/css/default.css','','','');Context::loadFile($__tmp,'','','');unset($__tmp); ?><?php } ?>
<?php if($__Context->layout_info->colorset=='white'){ ?><!--#Meta:layouts/xe_official/css/white.css--><?php $__tmp=array('layouts/xe_official/css/white.css','','','');Context::loadFile($__tmp,'','','');unset($__tmp); ?><?php } ?>
<?php if($__Context->layout_info->colorset=='black'){ ?><!--#Meta:layouts/xe_official/css/black.css--><?php $__tmp=array('layouts/xe_official/css/black.css','','','');Context::loadFile($__tmp,'','','');unset($__tmp); ?><?php } ?>
...
<div class="xe">
	<div class="header">
		...
		<div class="language">
			...
			<ul class="selectLang">
				<?php if($__Context->lang_supported&&count($__Context->lang_supported))foreach($__Context->lang_supported as $__Context->key=>$__Context->val){ ?><?php if($__Context->key!= $__Context->lang_type){ ?><li><button type="button" onclick="doChangeLangType('<?php echo $__Context->key ?>');return false;"><?php echo $__Context->val ?></button></li><?php }} ?>
			</ul>
		</div>
		<div class="gnb">
			<ul>
		...
		<form action="<?php echo getUrl() ?>" method="post" class="iSearch"><input type="hidden" name="error_return_url" value="<?php echo getRequestUriByServerEnviroment() ?>" />
	...
	<div class="body">
		<div class="lnb">
			...
			<?php if($__Context->main_menu->list&&count($__Context->main_menu->list))foreach($__Context->main_menu->list as $__Context->key1=>$__Context->val1){ ?><?php if($__Context->val1['selected']){ ?><h2><a href="<?php echo $__Context->val1['href'] ?>"<?php if($__Context->val1['open_window']=='Y'){ ?> target="_blank"<?php } ?>><?php echo $__Context->val1['link'] ?></a></h2><?php }} ?>
			<?php if($__Context->main_menu->list&&count($__Context->main_menu->list))foreach($__Context->main_menu->list as $__Context->key1=>$__Context->val1){ ?><?php if($__Context->val1['selected'] && $__Context->val1['list']){ ?><ul class="locNav">
