...
<div class="table even">
	<form class="form"><input type="hidden" name="error_return_url" value="<?php echo getRequestUriByServerEnviroment() ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
		<table width="100%" border="1" cellspacing="0">
			<caption>
				...
				<span class="side">
					<span class="btn"><a href="<?php echo getUrl('act','dispBoardAdminInsertBoard','module_srl','') ?>"><?php echo $__Context->lang->cmd_create_board ?>...</a></span>
					<span class="btn"><a href="#manageSelectedModule" class="modalAnchor"><?php echo $__Context->lang->cmd_manage_selected_board ?>...</a></span>
				</span>
			</caption>
			<thead>
				<tr>
				...
				<?php if($__Context->board_list&&count($__Context->board_list))foreach($__Context->board_list as $__Context->no=>$__Context->val){ ?><tr>
					...
					<td class="nowr">
						<?php if(!$__Context->val->module_category_srl){ ?>
							<?php if($__Context->val->site_srl){ ?><?php echo $__Context->lang->virtual_site ?><?php } ?>
							<?php if(!$__Context->val->site_srl){ ?><?php echo $__Context->lang->not_exists ?><?php } ?>
						<?php } ?>
						<?php if($__Context->val->module_category_srl){ ?><?php echo $__Context->module_category[$__Context->val->module_category_srl]->title ?><?php } ?>
					</td>
					...
					<td class="nowr"><a href="<?php echo getUrl('act','dispBoardAdminBoardInfo','module_srl',$__Context->val->module_srl) ?>" title="<?php echo $__Context->lang->cmd_setup ?>"><?php echo $__Context->lang->cmd_setup ?></a></td>
					<td class="nowr"><a href="<?php echo getUrl('','module','module','act','dispModuleAdminCopyModule','module_srl',$__Context->val->module_srl) ?>" onclick="popopen(this.href);return false;" title="<?php echo $__Context->lang->cmd_copy ?>"><?php echo $__Context->lang->cmd_copy ?></a></td>
					<td class="nowr"><a href="<?php echo getUrl('act','dispBoardAdminDeleteBoard','module_srl', $__Context->val->module_srl) ?>" title="<?php echo $__Context->lang->cmd_delete ?>"><?php echo $__Context->lang->cmd_delete ?></a></td>
