...
<div class="boardRead">
    <div class="originalContent">
        <div class="readHeader">
            <div class="titleAndUser">
                ...
                <div class="title">
                    <<?php echo $__Context->module_info->document_title_format ?>><a href="<?php echo $__Context->oDocument->getPermanentUrl() ?>"><div style="color:#<?php echo $__Context->module_info->titlecolor ?>"><?php echo $__Context->oDocument->getTitle() ?></div></a></<?php echo $__Context->module_info->document_title_format ?>>
                </div>

                <?php if($__Context->module_info->display_author!='N'){ ?>
                <div class="userInfo">
                    <?php if(!$__Context->oDocument->getMemberSrl()){ ?>
                        <div class="author" style="color:#<?php echo $__Context->module_info->namecolor ?>">
                        <?php if($__Context->oDocument->isExistsHomepage()){ ?>
                            <a href="<?php echo $__Context->oDocument->getHomepageUrl() ?>" onclick="window.open(this.href);return false;" style="color:#<?php echo $__Context->module_info->namecolor ?>"><?php echo $__Context->oDocument->getNickName() ?></a>
                        <?php }else{ ?>
                            <?php echo $__Context->oDocument->getNickName() ?>
                        <?php } ?>
                        </div>
                    <?php }else{ ?>
                        <div class="author" style="color:#<?php echo $__Context->module_info->namecolor ?>"><span class="member_<?php echo $__Context->oDocument->get('member_srl') ?>"><?php echo $__Context->oDocument->getNickName() ?></span></div>
                    <?php } ?>
                </div>
                <?php } ?>
            ...
            <div class="dateAndCount">
                ...
                <div class="date" title="<?php echo $__Context->lang->regdate ?>" style="color:#<?php echo $__Context->module_info->datecolor ?>">
                ...
                <div class="readedCount" title="<?php echo $__Context->lang->readed_count ?>" style="color:#<?php echo $__Context->module_info->readedcountcolor ?>"><?php echo $__Context->oDocument->get('readed_count') ?></div>

                <?php if($__Context->oDocument->get('voted_count')!=0 || $__Context->oDocument->get('blamed_count')!=0){ ?>
                <div class="votedCount" title="<?php echo $__Context->lang->voted_count ?>" style="color:#<?php echo $__Context->module_info->votedcountcolor ?>">
                    <strong><?php echo $__Context->oDocument->get('voted_count') ?> / <?php echo $__Context->oDocument->get('blamed_count') ?></strong>
                </div>
                <?php } ?>
                ...
                <?php if($__Context->oDocument->isSecret() && !$__Context->oDocument->isGranted()){ ?>
                    <div class="secretContent">
                        ...
                        <input type="hidden" name="document_srl" value="<?php echo $__Context->oDocument->document_srl ?>" />
                            ...
                            <div class="content"><input type="password" name="password" id="cpw" class="inputTypeText" /><span class="button <?php echo $__Context->btn_class ?>"><input type="submit" value="<?php echo $__Context->lang->cmd_input ?>" accesskey="s" /></span></div>
    ...
    <div class="contentButton">
        ...
        <a href="<?php echo getUrl('document_srl','') ?>" class="button <?php echo $__Context->btn_class ?>"><span><?php if($__Context->module_info->default_style != 'blog'){ ?><?php echo $__Context->lang->cmd_list ?><?php }else{ ?><?php echo $__Context->lang->cmd_view_all ?><?php } ?></span></a>
        <?php } ?>
        <?php if($__Context->oDocument->isEditable()){ ?>
        <a href="<?php echo getUrl('act','dispBoardWrite','document_srl',$__Context->oDocument->document_srl,'comment_srl','') ?>" class="button <?php echo $__Context->btn_class ?>"><span><?php echo $__Context->lang->cmd_modify ?></span></a>
        <a href="<?php echo getUrl('act','dispBoardDelete','document_srl',$__Context->oDocument->document_srl,'comment_srl','') ?>" class="button <?php echo $__Context->btn_class ?>"><span><?php echo $__Context->lang->cmd_delete ?></span></a>
        <?php } ?>
    </div>
...
<?php if($__Context->oDocument->allowTrackback()){ ?>
    <?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/xe_board_extended','trackback.html') ?>
<?php } ?>

<!-- 댓글 -->
<a name="comment"></a>
<?php $__tpl=TemplateHandler::getInstance();echo $__tpl->compile('modules/board/skins/xe_board_extended','comment.html') ?>
...
<?php if($__Context->grant->write_comment && $__Context->oDocument->isEnableComment() ){ ?>
    ...
    <input type="hidden" name="content" value="" />
        ...
        <div class="commentButton tRight">
            <span class="button <?php echo $__Context->btn_class ?>"><input type="submit" value="<?php echo $__Context->lang->cmd_comment_registration ?>" accesskey="s" /></span>
        </div>
    </form>
<?php } ?>
